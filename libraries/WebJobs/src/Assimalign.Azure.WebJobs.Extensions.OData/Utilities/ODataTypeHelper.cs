...
namespace Assimalign.Azure.WebJobs.Bindings.OData.Utilities
{
    ...
    using Assimalign.Azure.WebJobs.Bindings.OData.Formatter;
    using Assimalign.Azure.WebJobs.Bindings.OData.Interfaces;


    internal static class ODataTypeHelper
    {
        /// <summary>
        ...
        {
            if (ODataTypeHelper.IsValueType(clrType))
            {
                // value types are only nullable if they are Nullable<T>
                return ODataTypeHelper.IsGenericType(clrType) && clrType.GetGenericTypeDefinition() == typeof(Nullable<>);
            }
            else
            {
        ...
        {
            if (ODataTypeHelper.IsNullable(clrType))
            {
                return clrType;
            }
            else
        ...
        {
            Type elementType;
            ODataTypeHelper.IsCollection(clrType, out elementType);
        ...
        {
            Type elementType;
            return ODataTypeHelper.IsCollection(clrType, out elementType);
        }
        ...
        public static bool IsCollection(Type clrType, out Type elementType)
        {
            if (clrType == null)
            {
                throw ODataError.ArgumentNull("clrType");
            }
            ...
            Type collectionInterface
                = clrType.GetInterfaces()
                    .Union(new[] { clrType })
                    .FirstOrDefault(
                        t => ODataTypeHelper.IsGenericType(t)
        ...
        internal static bool IsIQueryable(Type type)
        {
            return type == typeof(IQueryable) ||
                (type != null && ODataTypeHelper.IsGenericType(type) && type.GetGenericTypeDefinition() == typeof(IQueryable<>));
        }
        ...
        internal static bool IsQueryPrimitiveType(Type type)
        {
            ...
            return ODataTypeHelper.IsEnum(type) ||
                   ODataTypeHelper.IsPrimitive(type) ||
                   type == typeof(Uri) ||
