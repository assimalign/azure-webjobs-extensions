...
namespace Assimalign.Azure.WebJobs.Bindings.OData.Formatter.Deserialization
{
    ...
    internal static class CollectionDeserializationHelpers
    {
            ...
            Type resourceType, string propertyName, Type propertyType)
        {
            ...
            if (list == null)
            {
                ...
                if (addMethod == null)
                {
                    string message = ODataError.Format(SRResources.CollectionShouldHaveAddMethod, propertyType.FullName, propertyName, resourceType.FullName);
                    throw new SerializationException(message);
                }
            }
            else if (list.GetType().IsArray)
            {
                string message = ODataError.Format(SRResources.GetOnlyCollectionCannotBeArray, propertyName, resourceType.FullName);
                throw new SerializationException(message);
            }
