...
namespace Assimalign.Azure.WebJobs.Bindings.OData.Builder
{
    ...
    public class ActionConfiguration : OperationConfiguration
    {
        ...
        /// </summary>
        public ActionConfiguration HasActionLink(Func<ODataResourceContext, Uri> actionLinkFactory, bool followsConventions)
        {
            if (actionLinkFactory == null)
            {
                throw ODataError.ArgumentNull("actionLinkFactory");
            }

            if (!IsBindable || BindingParameter.TypeConfiguration.Kind != EdmTypeKind.Entity)
            {
                throw ODataError.InvalidOperation(SRResources.HasActionLinkRequiresBindToEntity, Name);
            }
        ...
        public Func<ODataResourceContext, Uri> GetActionLink()
        {
